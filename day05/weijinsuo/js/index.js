...
var initMobileTab = function () {
    /*1. 解决换行问题*/
    var $navTabs = $('.wjs_products .nav-tabs')
    var width = 0;
    $navTabs.find('li').each(function (i, item) {
        var $currLi = $(this) // or $(item)
        /*
        * width()  内容宽度
        * innerWidth()  内容+内边距
        ...
        * */
        var liWidth = $currLi.outerWidth(true) // 拿到li的宽度
        width += liWidth
    })
    ...
    $navTabs.width(width)
    /*2. 修改结构使之 区域滑动的结构*/
    // 加父容器( .nav-tabs-parent )给 .nav-tabs  叫 nav-tabs-parent

    /*3. 自己实现滑动效果 使用iscroll*/
    /*找到大容器*/
    var domParent = $('.nav-tabs-parent')[0];
    new IScroll(domParent,{
        scrollX:true,
        scrollY:false,
        click:true
    })
